...
生成config文件：  
通过写盘config文件的方式启动内核是有bug的。生成和写盘config文件需要时间，如果这期间系统卡顿一下就有可能发生意外。比如连续启动1#, 2#两个服务器，1#服务器的config写盘然后启动，这时系统卡顿一下2#服务器没来得及写盘就启动了，于是2#服务器会读取到1#服务器的config，结果通常会是因为端口冲突而启动失败。要是通过stdin:方式传入config就不会发生这样的问题。为了降低这个bug的发生概率，V2RayGCon每次写入config后都要等待3秒钟，其实这个时间有时候是不够的，所以只是降低概率不是修复这个bug。另外这个时间也不可以设置得太长。因为3秒就意味着1个小时最多进行1200次测速，如果每个服务器测3次那么就是只能测400个服务器。不知道哪个大聪明把v2ray-core v5.x的从stdin:读取配置的功能改废了，现在只能从硬盘读取config文件。每小时1200次写盘啊！！！  
