...
测速模板：  
测速的原理是启动一个入口为HTTP协议的代理服务器，然后测量通过这个代理去下载网页所用的时间。不同core的配置格式有不同，所以不能直接套用xray/v2ray的HTTP inbound配置。用户可以给自定义core编写合适的HTTP inbound来支持测速功能。当然前提是core支持HTTP协议。配置编写方式参考软件自带的http模板。测速时%host%将替换为127.0.0.1, %port%替换为随机端口。为了兼容旧版本，测速模板有个“无”选项。它其实是xray/v2ray的HTTP inbound。  
